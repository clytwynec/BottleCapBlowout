...
class GS_EditorMenu(GS_MenuBase):
	...
	def Initialize(self):
		...
		self.mMenuImages["Edit_Level" + str(maxLevelNum) + ".lvl"] = self.mFont.render("New Level: Level" + str(maxLevelNum) + ".lvl", True, Colors.WHITE)
		self.mMenuRects["Edit_Level" + str(maxLevelNum) + ".lvl"] = self.mMenuImages["Edit_Level" + str(maxLevelNum) + ".lvl"].get_rect()
		self.mMenuRects["Edit_Level" + str(maxLevelNum) + ".lvl"].topleft = (100, 55)
